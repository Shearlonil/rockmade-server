...
const uploadSingleFile = multer({
    ...
    fileFilter: ( req, file, callback ) => {
        ...
        if ( file.mimetype !== mimetype['pdf'] && file.mimetype !== mimetype['doc'] && file.mimetype !== mimetype['docx']) {
    ...
    uploadSingleFile( req, res, err => {      
        ...
        if( err instanceof multer.MulterError || ( req.body.invalidFileFormat )) {
            return res.status( 400 ).json({ 
                status: "error", 
                // message: err
                message: `Invalid file format or file size too large. Please upload a PDF or DOCUMENT file not greater than 512KB.` 
            });
        }
